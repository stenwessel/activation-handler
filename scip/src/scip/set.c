...
SCIP_RETCODE SCIPsetCopyPlugins(
   ...
   SCIP_Bool             copyactivationhdlrs,/**< should the activation handlers be copied */
   ...
   )
{
   ...
   *allvalid = TRUE;
   ...
   /* copy all activation handler plugins */
   if( copyactivationhdlrs && sourceset->activationhdlrs != NULL )
   {
      for( p = sourceset->nactivationhdlrs - 1; p >= 0; --p )
      {
         SCIP_CALL( SCIPactivationhdlrCopyInclude(sourceset->activationhdlrs[p], targetset) );
      }
   }
   ...
   )
{
   ...
   (*set)->activationhdlrs = NULL;
   (*set)->activationhdlrssize = 0;
   (*set)->nactivationhdlrs = 0;
   ...
   )
{
   ...
   /* free activation handlers */
   for( i = 0; i < (*set)->nactivationhdlrs; ++i )
   {
      SCIP_CALL( SCIPactivationhdlrFree(&(*set)->activationhdlrs[i], *set) );
   }
   BMSfreeMemoryArrayNull(&(*set)->activationhdlrs);
...
/** inserts activation handler in activation handler list */
SCIP_RETCODE SCIPsetIncludeActivationhdlr(
   SCIP_SET*             set,                /**< global SCIP settings */
   ...
   )
{
   ...
   assert(!SCIPactivationhdlrIsInitialized(activationhdlr));

   if( set->nactivationhdlrs >= set->activationhdlrssize )
   {
      set->activationhdlrssize = SCIPsetCalcMemGrowSize(set, set->nactivationhdlrs + 1);
      SCIP_ALLOC( BMSreallocMemoryArray(&set->activationhdlrs, set->activationhdlrssize) );
   }
   assert(set->nactivationhdlrs < set->activationhdlrssize);

   set->activationhdlrs[set->nactivationhdlrs] = activationhdlr;
   set->nactivationhdlrs++;

   return SCIP_OKAY;
}

/** returns the activation handler of the given name, or NULL if not existing */
SCIP_ACTIVATIONHDLR* SCIPsetFindActivationhdlr(
   SCIP_SET*             set,                /**< global SCIP settings */
   ...
   )
{
   ...
   for( i = 0; i < set->nactivationhdlrs; ++i )
   {
      if( strcmp(SCIPactivationhdlrGetName(set->activationhdlrs[i]), name) == 0 )
         return set->activationhdlrs[i];
   }
   ...
   )
{
   ...
   for( i = 0; i < set->nactivationhdlrs; ++i )
   {
      SCIP_CALL( SCIPactivationhdlrInit(set->activationhdlrs[i], set) );
   }
   ...
   )
{
   ...
   for( i = 0; i < set->nactivationhdlrs; ++i )
   {
      SCIP_CALL( SCIPactivationhdlrExit(set->activationhdlrs[i], set) );
   }
