   ...
   if ( SCIPgetNLPBranchCands(scip) > 0 )
   {
      ...
      *result = SCIP_DIDNOTFIND;
      ...
      for (c = 0; c < nconss; ++c)
      {
         ...
         /* when the constraint has an activation handler, we check whether this constraint is indeed active */
         if( cons->activationhdlr != NULL )
         {
            SCIP_Bool isActive;
            SCIP_CALL( SCIPactivationhdlrIsActive(cons->activationhdlr, cons, scip->set, &isActive) );
         ...
         SCIP_CALL( separateInequalities(scip, result, cons, consdata->nrows, consdata->vars1, consdata->vars2, vals1, vals2) );
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   for (c = 0; c < nconss; ++c)
   {
      ...
      SCIP_CALL( propVariables(scip, cons, &infeasible, &found, &ngen) );
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for (c = 0; c < nconss; ++c)
   {
      ...
      SCIP_CALL( propVariables(scip, cons, &infeasible, &found, &curngen) );
   ...
   )
{
   ...
   else
   {
      ...
      SCIP_CALL( SCIPcreateCons(scip, cons, name, conshdlr, consdata, NULL, initial, separate, enforce, check,
            propagate,
