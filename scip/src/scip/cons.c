...
SCIP_RETCODE SCIPconsCreate(
   ...
   SCIP_ACTIVATIONHDLR*  activationhdlr,     /**< activation handler for this constraint */
   ...
   )
{
   ...
   (*cons)->activationhdlr = activationhdlr;
   ...
   )
{
   ...
   else
   {
      ...
      else
      {
         ...
         SCIP_CALL( SCIPconsCreate(transcons, blkmem, set, origcons->name, origcons->conshdlr, origcons->consdata,
               origcons->activationhdlr, origcons->initial, origcons->separate, origcons->enforce, origcons->check,
               origcons->propagate, origcons->local, origcons->modifiable, origcons->dynamic, origcons->removable,
               origcons->stickingatnode, FALSE, FALSE) );
      }
...
/** sets the activation handler of the given constraint */
SCIP_RETCODE SCIPconsSetActivationhdlr(
   SCIP_CONS*            cons,               /**< constraint */
...
#undef SCIPconsGetActivationhdlr
...
/** returns the activation handler of the constraint */
SCIP_ACTIVATIONHDLR* SCIPconsGetActivationhdlr(
   SCIP_CONS*            cons                /**< constraint */
   )
{
   assert(cons != NULL);

   return cons->activationhdlr;
}
