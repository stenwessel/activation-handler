# Activation handler guide
This repository contains the Python interface from SCIP, modified to contain API enpoints for the activation handler plugins.
It assumes [SCIP (with the activation handlers)](https://gitlab.tue.nl/master-project-sten-wessel/scip) has been installed in the directory stored in the `SCIPOPTDIR` environment variable.
For installation instructions of the PySCIPOpt interface, see the original README below.
